...
module Ctx = struct
  ...
  = fun () -> begin
    ...
    let budget_ms = args.z3Timeout * 1000 in
...
module Expr = struct
  ...
  let sort_of_void : unit -> Sort.t
  = fun () -> begin
    (* sort_of_void function start *)
    Z3.Tuple.mk_sort (Ctx.read ()) (Symbol.create "void") [] []
    (* sort_of_void function start *)
  end

  let sort_of_int : unit -> Sort.t
  = fun () -> begin
    (* sort_of_int function start *)
    Z3.Arithmetic.Integer.mk_sort (Ctx.read ())
    (* sort_of_int function end *)
  end

  let sort_of_bool : unit -> Sort.t
  = fun () -> begin
    (* sort_of_bool function start *)
    Z3.Boolean.mk_sort (Ctx.read ())
    (* sort_of_bool function end *)
  end
...
module Solver = struct
  ...
  = fun () -> begin
    ...
    (* _create function end *)
  end

  let _formula_add : t -> Fmla.t list -> unit
  = fun s1 fl2 -> begin
    (* _formula_add function start *)
    Z3.Solver.add s1 fl2
    (* _formula_add function end *)
  end
